function [p_L,p_R,r_L,r_R] = Calibrate(CalData_L,CalData_R,CalPoints_L,CalPoints_R,axis)
...
if axis == 1
    ...
    while ReselectCalibMean ~= 3
        ...
        hold on
        for i = 1:length(CalData_L)
            plot(CalData_L{i}(3,:),color{i})
        end
        title('Left Eye  Vertical')
        ylim([-5 5])
        ylabel('Volts')
        xlabel('Samples')
        ...
        hold on
        for i = 1:length(CalData_R)
            plot(CalData_R{i}(5,:),color{i})
        end
        title('Right Eye Vertical')
        ylim([-5 5])
        ylabel('Volts')
        xlabel('Samples')
        ...
        LE_MeanCalibVolts(1,:) = [mean(CalData_L{1}(3,LEpt1:LEpt2)),...
            mean(CalData_L{2}(3,LEpt1:LEpt2)),...
            mean(CalData_L{3}(3,LEpt1:LEpt2))];
        ...
        subplot(2,2,4)
        cla
        RE_MeanCalibVolts(1,:) = [mean(CalData_R{1}(5,REpt1:REpt2)),...
            mean(CalData_R{2}(5,REpt1:REpt2)),...
            mean(CalData_R{3}(5,REpt1:REpt2))];
        [RE_R,RE_PVAL] = corrcoef(RE_MeanCalibVolts(1,:),CalPoints_R);
        RE_CorrCoef(1,:) = RE_R(1,2);
        RE_CalibFit(1,:) = polyfit(RE_MeanCalibVolts(1,:),CalPoints_R,1);
        f = polyval(RE_CalibFit(1,:),RE_MeanCalibVolts(1,:));
        ...
        % Menu
        ReselectCalibMean = menu('Reselect data to calculate calibration mean',...
            'Left Eye Vert','Right Eye Vert','Done');
