...
%   Copyright 1993-2014 The MathWorks, Inc.
  ...
  % migrate IMSHOW settings
  images.internal.migrateImshowSettings();
  s = Settings;
  ...
  if ~initial_mag_specified
      style = s.matlab.imshow.InitialMagnificationStyle;
      if strcmp(style,'numeric')
          initial_mag = s.matlab.imshow.InitialMagnification;
      else
          initial_mag = style;
      end
  else
  ...
  % running MOTW.
  dockedInMOTWContext = s.matlab.imshow.AlwaysUseFitMagnification;
...
function border = getBorder(specific_args)
  ...
  if ~border_specified
      s = Settings;      
      border = s.matlab.imshow.BorderStyle;
  else
